...
const [backface_framebuffer, backface_texture, backface_renderbuffer] =
    create_framebuffer(ctx.gl, ctx.gl.canvas.width, ctx.gl.canvas.height);
...
void main(){
    vec3 skybox = sample_lat_long(envmap_sky, normalize(-1.0*position)).rgb;
    skybox = pow(skybox, vec3(1.0/2.2));
    frag_color = vec4(skybox, 1);
}`);
...
void main(){
    ...
    vec3 reflect_col = sample_lat_long(envmap_sky, reflect_dir).rgb;

    float ior = 1.0 / 1.33 ;
    vec3 refract_dir = refract(-view_dir, normal, ior);
    vec3 refract_col = sample_lat_long(envmap_sky, refract_dir).rgb;

    vec2 uv = gl_FragCoord.xy / vec2(textureSize(framebuffer_texture, 0));
    vec3 backface = texture(framebuffer_texture, uv).rgb;

    frag_color = vec4(backface + pow(reflect_col, vec3(1.0/2.2)), 1);
}`);
ctx.shaders["shader_water_backface"] = ctx.create_shader(`#version 300 es
layout(location = 0) in vec3 position_attrib;
layout(location = 1) in vec3 normal_attrib;
...
void main(){
    ...
    frag_color = vec4(pow(reflect_col, vec3(1.0/2.2)), 1);
}`);
...
function resize_event(ctx){
    ...
    resize_framebuffer(
        ctx.gl,
        backface_framebuffer,
        backface_texture,
        backface_renderbuffer,
        ctx.gl.canvas.width, ctx.gl.canvas.height
    );
...
// scene_snells_window
let water_cube = ctx.create_drawable(
            "shader_water",
            create_box(3, 3, 3),
            [1, 0, 1],
            translate_3d([0, 0, 0])
...
function update(current_time){
    ...
    for(let scene_id in ctx.scenes){
        ...
        if(scene_id == "scene_charges"){
            ...
            ctx.gl.bindFramebuffer(ctx.gl.FRAMEBUFFER, backface_framebuffer);
            gl.scissor(0, 0, gl.canvas.width, gl.canvas.height);
            gl.useProgram(ctx.shaders["shader_water_backface"].program);
            gl.activeTexture(gl.TEXTURE0);
            gl.bindTexture(gl.TEXTURE_2D, textures["envmap_sky"].texture);
            gl.clearColor(0, 0, 0, 0);
            gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);
            water_cube.shader = "shader_water_backface";
            gl.cullFace(gl.FRONT);
            ctx.draw(water_cube, {}, null, "shader_water_backface");

            water_cube.shader = "shader_water";
            ctx.gl.bindFramebuffer(ctx.gl.FRAMEBUFFER, null);
            gl.cullFace(gl.BACK);
            ...
            gl.activeTexture(gl.TEXTURE1);
            gl.bindTexture(gl.TEXTURE_2D, backface_texture);
            gl.generateMipmap(gl.TEXTURE_2D);
            gl.uniform1i(ctx.shaders["shader_water"].uniforms["framebuffer_texture"].location, 1);

            gl.enable(gl.BLEND);
            gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
            gl.cullFace(gl.FRONT);
            ctx.draw(water_cube, {"backface": 1.0});
            gl.cullFace(gl.BACK);
            ctx.draw(water_cube, {"backface": 0.0});
