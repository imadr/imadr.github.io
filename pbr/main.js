...
const path_points = ordered_idx.map(i => vertices[i]);
...
function get_position_along_path(t) {
    if (t < 0) t = 0;
    if (t > 1) t = 1;
    ...
    const p0 = path_points[segment];
    const p1 = path_points[segment + 1];
    return [
...
for(let i = 0; i < num_electrons; i++){
    ...
    let t = i/num_electrons;
    coil_electrons.push([coil_electron, t]);
}
// scene_induction setup
...
function update(current_time){
    ...
    for(let scene_id in ctx.scenes){
        ...
        if(scene_id == "scene_charges"){
            ...
            for(let i = 0; i < num_electrons; i++){
                coil_electrons[i][1] += delta_time*0.01;
                if(coil_electrons[i][1] > 1){
                    coil_electrons[i][1] = 0;
                }

                coil_electrons[i][0].transform = mat4_mat4_mul(
                    translate_3d(get_position_along_path(coil_electrons[i][1])),
                    coil_transform
                );
                ctx.draw(coil_electrons[i][0]);
            }
            ...
            let stuff_speed = 0.08*2; // this needs to be a multiple of 2 (?) for some reason
